...
def get_db_path(page):
    if page.platform in [Platform.ANDROID, Platform.IOS]:
        return os.path.join(os.path.expanduser("~"), "data.db")
    return os.path.join(os.path.expanduser("~"), "data.db")
...
def main(page: Page):
    ...
    is_mobile = page.width < 600 or page.platform in [Platform.ANDROID, Platform.IOS]
    
    if page.client_storage.get("db_path"):
        db_path = page.client_storage.get("db_path")
    else:
        db_path = get_db_path(page)
        page.client_storage.set("db_path", db_path)
    ...
    def show_main(bg="#041955", fg="#3450a1"):
        ...
        def on_file_result(e):
            if e.path:
                file_picker._result_path = e.path
        ...
        def export_db(e=None):
            ...
            async def task():
                def on_result(ev):
                    if ev.path:
                        try:
                            shutil.copyfile(db_path, ev.path)
                            page.snack_bar = SnackBar(Text("БД сохранена"), open=True)
                        except Exception as ex:
                            page.snack_bar = SnackBar(Text(f"Ошибка: {str(ex)}"), open=True)
                        page.update()

                file_picker.on_result = on_result
                await file_picker.save_file(file_name="data.db")

            page.run_task(task)

        def import_db(e=None):
            close_drawer()
            ...
            async def task():
                def on_result(ev):
                    ...
                    try:
                        ...
                        page.snack_bar = SnackBar(Text("БД загружена"), open=True)
                        refresh()
                    except Exception as ex:
                        page.snack_bar = SnackBar(Text(f"Ошибка: {str(ex)}"), open=True)
                    page.update()
        ...
        def export_db_cloud(e=None):
            ...
            try:
                ...
                page.client_storage.set("db_backup", json_str)
                page.snack_bar = SnackBar(Text("БД сохранена в памяти"), open=True)
                page.update()
            except Exception as ex:
                page.snack_bar = SnackBar(Text(f"Ошибка: {str(ex)}"), open=True)
                page.update()

        def import_db_cloud(e=None):
            close_drawer()
            
            try:
                json_str = page.client_storage.get("db_backup")
                if not json_str:
                    page.snack_bar = SnackBar(Text("Нет сохранённой БД"), open=True)
                    page.update()
                    return
                ...
                conn.commit()
                page.snack_bar = SnackBar(Text("БД восстановлена"), open=True)
                refresh()
                page.update()
            except Exception as ex:
                page.snack_bar = SnackBar(Text(f"Ошибка: {str(ex)}"), open=True)
        ...
        drawer_panel = Container(
            ...
            content=Column([
                ...
                ListTile(
                    ...
                    subtitle=Text("Сохранить в Downloads", color="white54", size=11),
                    on_click=export_db,
                ),
                ListTile(
                    leading=Icon(Icons.DOWNLOAD, color="white"),
                    title=Text("Загрузить БД", color="white"),
                    subtitle=Text("Взять из Downloads", color="white54", size=11),
                    on_click=import_db,
                ),
                Divider(color="white24"),
                ListTile(
                    leading=Icon(Icons.CLOUD_UPLOAD, color="white"),
                    title=Text("Сохранить БД", color="white"),
                    subtitle=Text("В память приложения", color="white54", size=11),
                    on_click=export_db_cloud,
                ),
                ListTile(
                    leading=Icon(Icons.CLOUD_DOWNLOAD, color="white"),
                    title=Text("Восстановить БД", color="white"),
                    subtitle=Text("Из памяти приложения", color="white54", size=11),
                    on_click=import_db_cloud,
                ),
            ])
        )
